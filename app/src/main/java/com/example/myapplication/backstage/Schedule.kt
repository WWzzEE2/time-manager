...
class Schedule(private val context: Context, testData: TestDataConfig? = null) {
        ...
        if (testData != null) {
            ...
            for (i in 0 until testData.ddlCnt)
                addDDl(
                    DDlInfo(
                        "Test ddl$i",
                        i.toLong(),
                        abs(rand.nextLong()) % (weekSec * testData.totWeek) + termStartTime,
                        "This is DDL $i",
                        termStartTime
                    )
                )
            for (i in 0 until testData.courseTryCnt) {
                val course = CourseInfo(
                    "Test Course$i",
                    termStartTime,
                    weekSec * testData.totWeek + termStartTime,
                    ArrayList(),
                    "This is Course $i",
                    "Classroom $i"
                )
    ...
    fun getTemplate(time: Long): CourseTemplate? =
        getTemplate(getDay(time), getHour(time), getWeek(time))
     ...
     */
    fun getDDl(week: Int, day: Int): List<DDlInfo> =
        (termStartTime + 1L * (week * 7 + day) * 24 * 3600 * 1000).let {
